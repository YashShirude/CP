...
def solve():
    ...
    sorted_corals = sorted(corals)
    water_required = 0

    for i in range(1,n):
        curr_num = sorted_corals[i]
        prev_num = sorted_corals[i-1]
        new_water_required = (i * (curr_num - prev_num))

        if water_required + new_water_required > w:
            extra_height = (w - water_required) // i
            print(sorted_corals[i-1] + extra_height)
            return
        
        water_required += new_water_required
    
    extra_height = (w - water_required) // n
    print(sorted_corals[n-1] + extra_height)
