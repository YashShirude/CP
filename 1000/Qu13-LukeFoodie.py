...
def solve():
    ...
    for i in arr:
        min_arr.append(abs(i-x))
        max_arr.append(i+x)

    upper = max_arr[0]
    lower = min_arr[0]

    for upper_next, lower_next in zip(max_arr, min_arr):
        if (lower <= upper_next and lower >= lower_next) or (lower_next <= upper and lower_next >= lower):
            lower = max(lower, lower_next)
            upper = min(upper,upper_next)
        else:
            lower = lower_next
            upper = upper_next
            changes += 1
