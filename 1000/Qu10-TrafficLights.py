...
def solve():
    ...
    index = 0
    result = 0
    for char in double_seq:
        if index >= n and light_index == -1:
            print(str(result) + "\n")
            return
        if char == light and light_index == -1:
            light_index = index
        if char == 'g' and light_index != -1:
            result = max(result, index - light_index)
            light_index = -1
        index += 1
    return
